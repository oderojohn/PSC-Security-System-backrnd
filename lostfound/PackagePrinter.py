    ...
    def print_lost_receipt(self, lost_item):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as printer:
                ...
                printer.sendall(self.BOLD_ON + b"LOST ITEM REPORT RECEIPT\n")
                printer.sendall(self.BOLD_OFF + self.LINE_FEED)

                printer.sendall(self.CENTER_ALIGN + self.BOLD_ON + self.QUAD_SIZE_ON)
                printer.sendall(f"{lost_item.tracking_id}\n".encode('utf-8'))
                printer.sendall(self.NORMAL_SIZE + self.BOLD_OFF + self.LINE_FEED)
                ...
                printer.sendall(self.BOLD_OFF + b"-----------------------------\n")
                printer.sendall(self.BOLD_ON + b"Type: ")
                printer.sendall(self.BOLD_OFF + str(lost_item.type).encode('utf-8') + self.LINE_FEED)

                if lost_item.item_name:
                    printer.sendall(self.BOLD_ON + b"Item Name: ")
                    printer.sendall(self.BOLD_OFF + str(lost_item.item_name).encode('utf-8') + self.LINE_FEED)

                if lost_item.card_last_four:
                    printer.sendall(self.BOLD_ON + b"Card Last Four: ")
                    printer.sendall(self.BOLD_OFF + str(lost_item.card_last_four).encode('utf-8') + self.LINE_FEED)

                printer.sendall(self.BOLD_ON + b"Description: ")
                printer.sendall(self.BOLD_OFF + str(lost_item.description or '').encode('utf-8') + self.LINE_FEED)

                printer.sendall(self.BOLD_ON + b"Place Lost: ")
                printer.sendall(self.BOLD_OFF + str(lost_item.place_lost or '').encode('utf-8') + self.LINE_FEED)

                printer.sendall(b"\n" + self.BOLD_ON + b"Reporter Information\n")
                printer.sendall(self.BOLD_OFF + b"-----------------------------\n")
                printer.sendall(self.BOLD_ON + b"Name: ")
                printer.sendall(self.BOLD_OFF + str(lost_item.owner_name or '').encode('utf-8') + self.LINE_FEED)

                if lost_item.reporter_phone:
                    printer.sendall(self.BOLD_ON + b"Phone: ")
                    printer.sendall(self.BOLD_OFF + str(lost_item.reporter_phone).encode('utf-8') + self.LINE_FEED)

                if lost_item.reporter_email:
                    printer.sendall(self.BOLD_ON + b"Email: ")
                    printer.sendall(self.BOLD_OFF + str(lost_item.reporter_email).encode('utf-8') + self.LINE_FEED)

                printer.sendall(b"\n" + self.BOLD_ON + b"Date Reported: ")
                printer.sendall(self.BOLD_OFF + lost_item.date_reported.strftime("%Y-%m-%d %H:%M").encode('utf-8') + self.LINE_FEED)

                printer.sendall(self.BOLD_ON + b"Status: ")
                printer.sendall(self.BOLD_OFF + str(lost_item.status).encode('utf-8') + self.LINE_FEED)
                ...
                printer.sendall(b"\n" * 2 + self.CENTER_ALIGN + self.BOLD_ON)
                printer.sendall(b"Thank you for reporting your lost item\n")
                printer.sendall(b"Handled by PSC ICT Department\n")
                printer.sendall(self.BOLD_OFF + b"\n")
